...
public class Inventory : MonoBehaviour
{
    ...
    public int maxCount; //макс кол-во элементов
    ...
    public void SearchForSameItem(Item item, int count)
    {
        for (int i=0;i<maxCount;i++)
        {
            ...
            {
                if(items[0].count < 128)//128 - кол-во в одной ячейке
                {
                    items[i].count += count;
    ...
    public void AddGraphics() //изображение
    {
        for (int i=0;i<maxCount; i++)
        {
            ...
            rt.localPosition = new Vector3(0, 0, 0); //текущее положение item
            rt.localScale = new Vector3(1, 1, 1); //размеры элемента item
            newItem.GetComponentInChildren<RectTransform>().localScale = new Vector3(1, 1, 1); //размер пока курсором держим
